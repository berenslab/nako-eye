...
def get_args():
    parser = argparse.ArgumentParser(
        description='Supervised training for classification from retinal images'
    )
    parser.add_argument(
        '--backbone',
        default='resnet18',
    ...
    parser.add_argument(
        '--epochs', default=25, type=int, help='number of epochs for training'
    )
    parser.add_argument(
        '--kfold',
        default=0,
...
def get_dataloader(args, feature_name):
    ...
    n_classes = len(dataset_train.mapping)

    return dataloader_train, dataloader_val, n_classes


def get_train_objs(args, n_classes):
    freeze = True if args.method == 'transfer' else False  # Freeze the backbone
...
if __name__ == '__main__':
    ...
    ##################### TRAINING PARAMS ######################
    experiment_name = (
        f'clf_{args.backbone_name}_{args.imagesize[0]}_{args.method}_{args.kfold}'
    )
    ...
    dataloader_train, dataloader_val, n_classes = get_dataloader(args, feature_name)
    model, uncompiled_model, optimizer, loss_fn = get_train_objs(args, n_classes)

    ###################### TRAINING LOOP #########################
    patience = 10 if args.method == 'scratch' else 5
    early_stopping = models.EarlyStopping(
        patience=patience, min_delta=0.01, model_file=model_file, verbose=True
    )
    ...
    for epoch in range(args.epochs):
        ...
        print(
            f'Epoch {epoch + 1}, train loss {loss_train[-1]:.3f}, val loss {loss_val[-1]:.3f}, {end_time - start_time:.1f} s'
        )
