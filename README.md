...
src/nako_eye/
  ...
  ├── regression_multitask/    # Blood pressure regression (systolic and diastolic, multitask)
...
scripts/
  ├── classification/          # Training scripts for sex classification
  └── regression/              # Training scripts for age/blood pressure regression
mplstyles/                     # Matplotlib styles for paper figures
```

## Usage
Install the package once (see Installation above), then run any module from the repo root:

```bash
python -m nako_eye.classification.train -h
python -m nako_eye.regression.train -h
```

See `scripts/classification/training_example.sh` for an example of 5-fold cross-validation for sex classification, and `scripts/regression/regression_example.sh` for age regression.
